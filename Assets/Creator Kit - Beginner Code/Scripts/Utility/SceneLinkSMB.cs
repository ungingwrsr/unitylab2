...
{
    /// <summary>
    /// A custom state machine behaviour that allow to store a reference to a Monobehaviour class linked to the animator.
    /// That allow  faster and more efficient access to it on state enter/exit callback (otherwise need a GetComponent
    /// each frame)
    /// </summary>
    /// <typeparam name="TMonoBehaviour">The type of the reference that will be kept</typeparam>
